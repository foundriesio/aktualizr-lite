...
docker compose --env-file=${docker_path}/.env.dev -f ${docker_path}/docker-compose.yml run $CCACHE_ARGS -e DEV_USER=$(id -u) -e DEV_GROUP=$(id -g) -e BASE_TARGET_VERSION=${BASE_TARGET_VERSION} -e USER_TOKEN=${USER_TOKEN} -e TAG=${TAG} -e E2E_TEST_OSTREE_TGZ="${E2E_TEST_OSTREE_TGZ}" -e E2E_TARGETS_LAYOUT="${E2E_TARGETS_LAYOUT}" -e SECONDARY_TAG=${SECONDARY_TAG} -e SECONDARY_BASE_TARGET_VERSION="${SECONDARY_BASE_TARGET_VERSION}" -e SECONDARY_E2E_TEST_OSTREE_TGZ="${SECONDARY_E2E_TEST_OSTREE_TGZ}" -e AKLITE_E2E_IMAGE=${AKLITE_E2E_IMAGE} aklite-e2e-test "$@"
