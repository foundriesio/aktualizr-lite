...
If `lmp-device-register` is used, `--start-daemon 0` is recommended
...
> [!TIP]
> You can find a containerized environment for a custom sota client development in
...
When the OSTree image is changed,
a reboot command is required after installing the update.
This is followed by the execution of the  `run` command to finalize the update process.
The exit code can be used to identify if a reboot is required.
...
can be used to automate an update flow like the one implemented by the main *aktualizr-lite* daemon,
while allowing for limited customizations.

This [sample bash script](https://raw.githubusercontent.com/foundriesio/sotactl/main/scripts/aklite-cli-example.sh)
...
One additional piece of information that the JSON output includes is if the Target is a failing Target.
This would mean the installation of the Target was attempted, but led to a rollback.
