...
def register_if_required():
    if not os.path.exists("/var/sota/client.pem"):
        ...
        if use_fioup:
            ...
            cmd = (
                f'DEVICE_FACTORY={factory_name} lmp-device-register --api-token "{api_token}" {token_header_args} '
...
def install_target(target: Target, explicit_version: bool = True, do_reboot: bool = True, do_finalize: bool = True):
    ...
    events_prefix: Set[Tuple[str, Optional[bool]]]
    if combined_steps:
        cmd = ['update']
        if explicit_version:
    ...
    if target.install_rollback:
        ...
        pending_events_prefix: Set[Tuple[str, Optional[bool]]] = events_prefix | ({('EcuDownloadStarted', None), ('EcuDownloadCompleted', True)} if use_fioup else set())
        ...
        if target.run_rollback:
            ...
            assert cp.returncode == ReturnCodes.Ok, cp.stdout.decode("utf-8")
            verify_callback([("install-final-pre", ""), ("install-post", "OK")])
            final_events_prefix: Set[Tuple[str, Optional[bool]]] = events_prefix | ({('EcuDownloadStarted', None), ('EcuDownloadCompleted', True)} if use_fioup else set())
