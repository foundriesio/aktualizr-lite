...
osf_token = os.getenv("USER_TOKEN")
if not osf_token:
...
# X - Pre-built, no apps (won't be used by tests)
# 0 - Small working ostree, no apps, starting point - $OSTREE_HASH_1
# 1 - Incomplete ostree (causes rollback) - $OSTREE_HASH_2
# 2 - Small working ostree with change, no apps - $OSTREE_HASH_3
# 3 - Add 1 app, keep ostree
# 4 - Add more apps
# 5 - Break 1 existing app (causes rollback)
# 6 - Change app, but it is still broken (causes rollback)
# 7 - Break build, non existing target
# 8 - Fixes build and app, success
# 9 - Update some app version
# 10 - Additional change to ostree - $OSTREE_HASH_4
# 11 - Broken ostree - $OSTREE_HASH_5
...
class Targets:
    ...
    def __init__(self, version_offset, install_rollback, run_rollback, build_error, ostree_image_version):
...
all_targets = {
    ...
    Targets.AddFirstApp: Targets(Targets.AddFirstApp, False, False, False, 3),
    Targets.AddMoreApps: Targets(Targets.AddMoreApps, False, False, False, 3),
    Targets.BreakApp: Targets(Targets.BreakApp, False, True, False, 3),
    Targets.UpdateBrokenApp: Targets(Targets.UpdateBrokenApp, False, True, False, 3),
    Targets.BrokenBuild: Targets(Targets.BrokenBuild, False, False, True, 3),
    Targets.FixApp: Targets(Targets.FixApp, False, False, False, 3),
    Targets.UpdateWorkingApp: Targets(Targets.UpdateWorkingApp, False, False, False, 3),
    Targets.UpdateOstreeWithApps: Targets(Targets.UpdateOstreeWithApps, False, False, False, 4),
    Targets.BrokenOstreeWithApps: Targets(Targets.BrokenOstreeWithApps, True, False, False, 5),
}
...
def register_if_required():
    if not os.path.exists("/var/sota/client.pem"):
        ...
        logger.info(f"Registering device... TOKEN={user_token}")
...
def verify_events(target_version, expected_events = None, second_to_last_corr_id = False):
    ...
    # print(json.dumps(d_update, indent=2))
...
def run_test_sequence_incremental():
    ...
    for target_version in install_sequence_incremental:
        ...
        logger.info(f"Updating to {target.version_offset} (target version={version}, {target.install_rollback=}, {target.run_rollback=}, {target.build_error=}, {target.ostree_image_version=}. {single_step=} {offline=}")
...
def run_test_sequence_update_to_latest():
    ...
    version = get_target_version(target.version_offset)
    requires_reboot = target.ostree_image_version != prev_ostree_image_version
    logger.info(f"Updating to latest target {target.version_offset} (target version={version}, {target.install_rollback=}, {target.run_rollback=}, {target.build_error=}, {target.ostree_image_version=}. {single_step=} {offline=}")
...
# TODO: Re-add apps shortlist testing
# TODO: Re-enable offline tests
