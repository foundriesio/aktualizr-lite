...
enum class PostInstallAction {
  Undefined = -1,
  NeedReboot,
  NeedRebootToUpdateBootFw,
  NeedDockerRestart,
  AlreadyInstalled
};
enum class PostRunAction { Undefined = -1, Ok, RollbackNeedReboot, NeedReboot, NoUpdateToRun, Failed };
...
const Uptane::Target getCurrentTarget(
    const Config& cfg_in, std::shared_ptr<HttpInterface> docker_client_http_client =
