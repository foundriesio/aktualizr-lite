...
class AppEngine : public Docker::RestorableAppEngine {
 public:
  AppEngine(boost::filesystem::path store_root, boost::filesystem::path install_root,
      ...
      : Docker::RestorableAppEngine(std::move(store_root), std::move(install_root), std::move(docker_root),
                                    std::move(registry_client), std::move(docker_client), std::move(client),
                                    std::move(docker_host), std::move(compose_cmd), std::move(storage_space_func),
                                    std::move(client_image_src_func), create_containers_if_install,
